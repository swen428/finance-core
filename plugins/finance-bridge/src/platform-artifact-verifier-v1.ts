...
function requireZeroNpmAudit(value: unknown, label: string): void {
  const audit = record(value, label);
  for (const severity of ["critical", "high", "moderate", "low", "total"] as const) {
...
async function verifyNpmAuditEvidence(params: {
  ...
  requireEqual(
    params.expectedArtifact[`${params.receiptPrefix}_audit_evidence_file`],
    expectedFileName,
    `${label} evidence file`,
  );
  ...
  const actualVulnerabilities = record(
  ...
  const expectedSummary = record(
    params.expectedArtifact[`${params.receiptPrefix}_audit`],
  ...
  for (const [receiptField, auditField] of [
    ...
    requireEqual(
      expectedSummary[receiptField],
      actualDependencies[auditField],
      `${label} ${receiptField}`,
    );
  }
...
function defaultCodeSignatureVerifier(path: string): void {
...
async function verifyBoundEntry(params: {
  pluginRoot: string;
  artifactEntries: ReadonlyMap<string, ArtifactHashResultV1["entries"][number]>;
  ...
  if (entry === undefined) throw new Error(`${params.label} is absent from the artifact.`);
  for (const field of ["sha256", "byte_count", "mode"] as const) {
...
export async function verifyPlatformArtifactReceiptV1(params: {
  ...
  const peerDependencies = record(
    packageJson.peerDependencies,
    "Finance plugin peer dependencies",
  );
  const peerDependenciesMeta = record(
    packageJson.peerDependenciesMeta,
    "Finance plugin peer dependency metadata",
  );
  const openClawPeerMeta = record(
    peerDependenciesMeta.openclaw,
    "Finance plugin OpenClaw peer metadata",
  );
  const devDependencies = record(
    packageJson.devDependencies,
    "Finance plugin development dependencies",
  );
  const expectedNodeVersion = `v${String(engines.node ?? "")}`;
  const environment: VerifierEnvironmentV1 = {
  ...
  requireEqual(
    sha256((await readRegularFile(pluginRoot, PLATFORM_PATCH)).bytes),
    patchHash,
    "platform patch sha256",
  );
  ...
  const staticAudit = record(
    supply.openclaw_static_security_audit,
    "OpenClaw static security audit",
  );
  requireEqual(staticAudit.evidence_scope, STATIC_AUDIT_SCOPE, "static audit evidence scope");
  requireEqual(
    staticAudit.evidence_file,
    STATIC_AUDIT.slice("platform/".length),
    "static audit evidence file",
  );
  ...
  const staticAuditPayload = record(
    JSON.parse(staticAuditBytes.toString("utf8")),
  ...
  requireEqual(
    staticAuditPayload.secretDiagnostics.length,
    staticAudit.secret_diagnostics,
    "static audit sensitive diagnostics",
  );
  requireEqual(staticAudit.secret_diagnostics, 0, "static audit sensitive diagnostics receipt");
  const expectedArtifact = record(
    supply.finance_plugin_artifact,
    "Finance plugin artifact receipt",
  );
  ...
  const dependencyBoundary = record(
    expectedArtifact.development_dependency_boundary,
    "Finance plugin development dependency boundary",
  );
  requireEqual(
    dependencyBoundary.runtime_peer_version,
    peerDependencies.openclaw,
    "runtime peer version",
  );
  requireEqual(dependencyBoundary.runtime_peer_optional, true, "runtime peer optional receipt");
  requireEqual(
    dependencyBoundary.runtime_package_installed,
    false,
    "runtime package installation receipt",
  );
  requireEqual(dependencyBoundary.compile_sdk_package, "openclaw-sdk", "compile SDK package");
  requireEqual(
    dependencyBoundary.compile_sdk_version,
    "2026.7.2-beta.6",
    "compile SDK version",
  );
  requireEqual(
    devDependencies["openclaw-sdk"],
    "npm:openclaw@2026.7.2-beta.6",
    "compile SDK alias",
  );
  ...
  const artifactEntries = new Map(
    params.artifact.entries.map((entry) => [entry.path, entry] as const),
  );
  if (params.artifact.entries.some((entry) =>
    entry.path === "node_modules/openclaw" || entry.path.startsWith("node_modules/openclaw/"))) {
  ...
  requireExactKeys(
    rootArtifact,
    ["package_version", "package_json_sha256", "npm_shrinkwrap_sha256"],
    "OpenClaw root artifact receipt",
  );
  requireExactKeys(
    aiArtifact,
    ["package_json_sha256", "npm_shrinkwrap_sha256"],
    "OpenClaw AI artifact receipt",
  );
  requireExactKeys(
    codexArtifact,
    ["package_json_sha256", "npm_shrinkwrap_sha256"],
    "OpenClaw Codex artifact receipt",
  );
  const combinedOpenClawArtifact = record(
    supply.combined_openclaw_artifact,
    "combined OpenClaw artifact receipt",
  );
  requireExactKeys(
    combinedOpenClawArtifact,
    ["policy_version", "artifact_sha256", "file_count", "byte_count"],
    "combined OpenClaw artifact receipt",
  );
  requireEqual(
    params.openclawArtifact.package_version,
    rootArtifact.package_version,
    "OpenClaw runtime package version",
  );
  for (const field of ["policy_version", "artifact_sha256", "file_count", "byte_count"] as const) {
    requireEqual(
      params.openclawArtifact[field],
      combinedOpenClawArtifact[field],
      `combined OpenClaw artifact ${field}`,
    );
  }
  const openclawEntries = new Map(
    params.openclawArtifact.entries.map((entry) => [entry.path, entry] as const),
  );
  requireEqual(
    openclawEntries.get("package.json")?.sha256,
    rootArtifact.package_json_sha256,
    "OpenClaw root package.json sha256",
  );
  requireEqual(
    openclawEntries.get("npm-shrinkwrap.json")?.sha256,
    rootArtifact.npm_shrinkwrap_sha256,
    "OpenClaw root npm-shrinkwrap sha256",
  );
  for (const [label, artifactReceipt, prefix] of [
    ...
    requireEqual(
      openclawEntries.get(`${prefix}/package.json`)?.sha256,
      artifactReceipt.package_json_sha256,
      `${label} package.json sha256`,
    );
    requireEqual(
      openclawEntries.get(`${prefix}/npm-shrinkwrap.json`)?.sha256,
      artifactReceipt.npm_shrinkwrap_sha256,
      `${label} npm-shrinkwrap sha256`,
    );
  }
  ...
  const buildProvenance = record(
    expectedArtifact.build_provenance,
  ...
  requireEqual(
    params.artifact.source_identity_sha256,
    buildProvenance.source_identity_sha256,
...
export async function executeReceiptBoundCompatibilityV1<TOutcome, TReceipt>(params: {
...
}): Promise<{ outcomes: TOutcome[]; receipt: TReceipt }> {
