...
type WholeCardField = (typeof FIELD_ORDER)[number];

const LABELS = new Map<string, WholeCardField>([
  ["amount", "amount"],
  ["金额", "amount"],
  ["currency", "currency"],
  ["币种", "currency"],
...
function containsUnsafeScalar(value: string, allowLineBreaks: boolean): boolean {
...
function splitLabel(line: string): [string, string] {
  ...
  if (positions.length === 0) throw new Error("Whole card line has no label separator.");
...
export function parseWholeCard(text: string): ParsedWholeCard {
  ...
  const parsed = new Map<WholeCardField, string>();
  for (const rawLine of text.split(/\r\n|[\n\r]/u)) {
    ...
    if (REFERENCE_LABELS.has(canonicalLabel)) {
      if (cardReference !== undefined) throw new Error("Whole card reference is duplicated.");
      cardReference = value;
      continue;
    }
    const field = LABELS.get(canonicalLabel);
    if (field === undefined) throw new Error("Whole card contains an unknown label.");
    if (parsed.has(field)) throw new Error("Whole card field is duplicated.");
  ...
  return {
    cardReference,
    fields: Object.fromEntries(FIELD_ORDER.map((field) => [field, parsed.get(field)!])) as WholeCardFields,
  };
}

function requireSafeLine(value: string, field: string): string {
...
export const EMPTY_WHOLE_CARD_FIELDS: WholeCardFields = {
  amount: "",
  currency: "",
  transaction_date: "",
  merchant: "",
