...
class ReceiptOutcomeUnknownError extends Error {
  constructor(readonly proposalPublicId: string) {
    super("Receipt command outcome is unknown.");
  }
}

function senderFromBinding(binding: PluginConversationBinding): string | undefined {
  if (typeof binding.data !== "object" || binding.data === null || Array.isArray(binding.data)) {
    return undefined;
  }
  const sender = (binding.data as Record<string, unknown>).senderId;
  return typeof sender === "string" ? sender : undefined;
}

function isPrivateOwnerContext(context: PluginCommandContext): boolean {
...
function response(text: string): PluginCommandResult {
  return { text, continueAgent: false };
}

function isRuntime(value: CommandAvailability): value is FinanceCommandRuntime {
...
function requireExactResult(result: JsonObject, fields: readonly string[]): void {
...
function requireString(result: JsonObject, field: string, pattern = SAFE_ID): string {
...
function requireHash(result: JsonObject, field: string): string {
  return requireString(result, field, HASH);
}

function requireNonNegativeInteger(result: JsonObject, field: string): number {
  const value = result[field];
...
function requireMoneyDisplay(result: JsonObject, field: string): string {
  return requireString(result, field, MONEY_DISPLAY);
}

function requireOk(response_: BridgeResponse): JsonObject {
  if (response_.status !== "ok") throw new Error("Bridge command refused.");
  return response_.result;
}

function requireStatusBinding(result: JsonObject, proposalPublicId: string): {
...
function tokenize(args: string | undefined): { verb: string; values: string[] } | undefined {
...
function requireProposal(value: string | undefined): string {
...
function requireSnapshotHash(value: string | undefined): string {
  if (value === undefined || !HASH.test(value)) throw new Error("Snapshot hash is invalid.");
  return value;
}

function requireCommandFilename(value: string | undefined): string {
...
async function readStatusBinding(
  runtime: FinanceCommandRuntime,
  proposalPublicId: string,
): Promise<{ proposalVersion: number; contentHash: string }> {
  const status = requireOk(await runtime.runner.run(createBridgeRequest(
    "get_status",
    {
...
async function renderReceiptOperation(
  proposalPublicId: string,
  invoke: () => Promise<BridgeResponse>,
  render: (result: JsonObject) => string,
...
function renderPrepared(result: JsonObject, proposalPublicId: string): string {
...
function renderFactSet(result: JsonObject): string {
  requireExactResult(result, [
    "identity_kind", "receipt_public_id", "fact_set_public_id", "fact_set_version",
    "fact_set_result_hash", "item_count", "allocation_count",
  ]);
  if (result.identity_kind !== "apply_fact_set") throw new Error("Fact-set result kind is invalid.");
...
function renderSnapshotReview(result: JsonObject, proposalPublicId: string): string {
  requireExactResult(result, [
  ...
  if (typeof shares !== "object" || shares === null || Array.isArray(shares) ||
      ...
      typeof shares[payer] !== "string" || !MONEY_DISPLAY.test(shares[payer] as string) ||
  ...
  return [
    ...
    `Payer ${payer} share: ${shares[payer] as string}`,
...
function renderAuthorization(result: JsonObject, expectedHash: string): string {
  requireExactResult(result, [
...
function renderFinalization(result: JsonObject, proposalPublicId: string): string {
  requireExactResult(result, [
...
function renderStatus(result: JsonObject, proposalPublicId: string): string {
  if (result.identity_kind !== "proposal" ||
      requireString(result, "proposal_public_id", PROPOSAL_ID) !== proposalPublicId ||
...
async function handleFinanceCommand(
  context: PluginCommandContext,
  availability: CommandAvailabilityProvider,
): Promise<PluginCommandResult> {
  const parsed = tokenize(context.args);
  if (parsed === undefined) return response("Use /finance bind, status, or unbind.");
  ...
  if (verb === "bind") {
    ...
    if (request.status === "pending") return { ...request.reply, continueAgent: false };
    if (request.status === "error") return response("Finance binding request was refused.");
  ...
  if (verb === "status") {
    ...
    if (!bound) return response("Finance bridge is not bound to this private conversation.");
    if (!isRuntime(currentAvailability)) return response("Finance bridge is not available.");
    const intakePublicId = values[0];
    if (!INTAKE_ID.test(intakePublicId)) return response("Finance intake reference is invalid.");
    const status = requireOk(await currentAvailability.runner.run(createBridgeRequest(
  ...
  if (verb === "prepare-receipt") {
    return renderReceiptOperation(
      proposalPublicId,
      () => runtime.runner.run(createBridgeRequest(
        "prepare_receipt_completion",
        { ...base, proposal_version: binding!.proposalVersion, content_hash: binding!.contentHash },
        `bridge-prepare-receipt:${proposalPublicId}`,
      ), COMMAND_DEADLINE_MS),
      (result) => renderPrepared(result, proposalPublicId),
    );
  }
  if (verb === "apply-facts") {
    return renderReceiptOperation(
      ...
      () => runtime.runner.run(createBridgeRequest(
        "apply_fact_set",
        { ...base, command_filename: filename! },
        `bridge-apply-fact-set:${proposalPublicId}`,
      ), COMMAND_DEADLINE_MS),
      renderFactSet,
    );
  }
  if (verb === "review-snapshot") {
    return renderReceiptOperation(
      ...
      () => runtime.runner.run(createBridgeRequest(
        ...
        base,
        `bridge-finalization-snapshot-review:${proposalPublicId}`,
      ), COMMAND_DEADLINE_MS),
      (result) => renderSnapshotReview(result, proposalPublicId),
    );
  }
  if (verb === "authorize") {
    return renderReceiptOperation(
      ...
      () => runtime.runner.run(createBridgeRequest(
        ...
        { ...base, expected_calculation_snapshot_hash: expectedHash! },
        `bridge-authorize-finalization:${proposalPublicId}`,
      ), COMMAND_DEADLINE_MS),
      (result) => renderAuthorization(result, expectedHash!),
    );
  }
  if (verb === "finalize") {
    return renderReceiptOperation(
      ...
      () => runtime.runner.run(createBridgeRequest(
        ...
        `bridge-finalize:${proposalPublicId}`,
      ), COMMAND_DEADLINE_MS),
      (result) => renderFinalization(result, proposalPublicId),
    );
  }
  const result = requireOk(await runtime.runner.run(createBridgeRequest(
    "get_status",
    { workspace_path: runtime.workspaceRoot, proposal_public_id: proposalPublicId },
  ), COMMAND_DEADLINE_MS));
  return response(renderStatus(result, proposalPublicId));
}

export function createFinanceCommand(availability: CommandAvailabilityProvider): OpenClawPluginCommandDefinition {
  return {
    name: "finance",
    ...
    handler: async (context) => {
      try {
        ...
        if (error instanceof ReceiptOutcomeUnknownError) {
          return response(
            `Receipt command outcome is unknown. Run /finance receipt-status ${error.proposalPublicId}.`,
          );
        }
        return response("Finance bridge is not available.");
      }
