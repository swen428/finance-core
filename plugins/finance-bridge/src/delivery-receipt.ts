...
export const FINANCE_DELIVERY_CAPABILITY =
  "telegram.finance-delivery-material-v1" as const;
...
export interface FinanceDeliveryReceiptV1 {
  ...
  consume(
    consumer: (material: FinanceDeliveryMaterialV1) => void | Promise<void>,
  ): Promise<void>;
}

export type FinanceDeliveryReceiptConsumerV1 = (
  receipt: FinanceDeliveryReceiptV1,
) => void | Promise<void>;
...
export type FinanceDeliveryPluginApi = OpenClawPluginApi & {
  ...
  registerFinanceDeliveryReceiptConsumerV1?: (
    consumer: FinanceDeliveryReceiptConsumerV1,
  ) => void;
};

export function requireFinanceDeliveryHost(
  api: OpenClawPluginApi,
): asserts api is FinanceDeliveryPluginApi & {
  readonly financeDeliveryCapabilities: readonly [typeof FINANCE_DELIVERY_CAPABILITY];
  registerFinanceDeliveryReceiptConsumerV1: (
    consumer: FinanceDeliveryReceiptConsumerV1,
  ) => void;
} {
  const candidate = api as FinanceDeliveryPluginApi;
