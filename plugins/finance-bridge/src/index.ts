...
import {
  ...
  getLoadedCodexPluginSourceV2,
  getLoadedOpenAIPluginSourceV2,
...
export function validateRetryCapability(value: unknown): void {
  if (typeof value !== "object" || value === null ||
      (value as {capabilities?: {maxRetries?: unknown}}).capabilities?.maxRetries !== true) {
    throw new Error("Pinned OpenClaw host does not prove effective maxRetries control.");
  }
}

function hostLlmRuntime(
  ...
  return {
    currentProjection: () => {
      ...
      return financeProjectionOrRefusalV2(
        reviewed.hostConfig,
        config.agentProfileV2,
        api.runtime.version,
        reviewed.codexPluginSource,
        reviewed.openaiPluginSource,
      );
    },
...
const defaultDependencies: RegistrationDependencies = {
  ...
  createHandoffPublisher: (config, markUnhealthy) => new HandoffPublisher(
    config.workspaceRoot,
    { markUnhealthy },
  ),
};

export function registerFinanceBridge(
  api: OpenClawPluginApi,
  ...
  if (api.rootDir === undefined) throw new Error("Loaded plugin root is unavailable.");
