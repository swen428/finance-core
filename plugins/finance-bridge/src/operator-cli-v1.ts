...
import {
  ...
  type ArtifactKind,
} from "./artifact-hash-v1.js";
import { validatePluginConfig, type FinanceBridgeConfig } from "./config.js";
...
import {
  ...
  createOperatorFailureV1,
  OperatorFailureV1,
  type OperatorFailureEnvelopeV1,
  verifyArtifactEvidenceV1,
  type CompatibilityLlmRuntimeV1,
} from "./model-compatibility-operator-v1.js";
import {
  executeReceiptBoundCompatibilityV1,
  verifyPlatformArtifactReceiptV1,
} from "./platform-artifact-verifier-v1.js";
...
function asOperatorFailure(
  error: unknown,
  details: Omit<OperatorFailureEnvelopeV1, "schema_version" | "elapsed_ms" | "timeout_triggered"> &
    Partial<Pick<OperatorFailureEnvelopeV1, "elapsed_ms" | "timeout_triggered">>,
): OperatorFailureV1 {
  if (error instanceof OperatorFailureV1) return error;
...
async function runOperatorPhase<T>(
  phase: OperatorFailureEnvelopeV1["phase"],
  category: OperatorFailureEnvelopeV1["category"],
...
const defaultDependencies: OperatorCliDependenciesV1 = {
...
function operatorRuntime(api: OpenClawPluginApi): CompatibilityLlmRuntimeV1 {
  return api.runtime.llm as unknown as CompatibilityLlmRuntimeV1;
}
...
export async function executeCompatibilityRegistrationV1(params: {
  ...
  let config: FinanceBridgeConfig;
  try {
  ...
  const initialHostRuntime = await runOperatorPhase(
    "config",
    "CONFIG_REFUSED",
    async () => resolveReviewedFinanceHostRuntimeV2(params.api.runtime),
  );
  ...
  const openclawArtifactRoot = await runOperatorPhase(
    "artifact",
    "ARTIFACT_REFUSED",
    async () => await dependencies.resolveOpenClawRoot(),
  );
  await runOperatorPhase("artifact", "ARTIFACT_REFUSED", () => {
    validateReviewedPluginSourcesV2(
      ...
      initialCodexPluginSource,
      initialOpenAiPluginSource,
    );
  });
  ...
  const artifacts = await runOperatorPhase("artifact", "ARTIFACT_REFUSED", async () => (
    await verifyArtifacts({ ...evidenceParams })
  ));
  const projection = await runOperatorPhase("config", "CONFIG_REFUSED", () => (
    deriveFinanceAgentProjectionV2(
      initialHostConfig,
      config.agentProfileV2,
      params.api.runtime.version,
      initialCodexPluginSource,
      initialOpenAiPluginSource,
    )
  ));
  ...
  const requireStableRuntimeEvidence = (params: {
    currentArtifacts: typeof artifacts;
  ...
  let assets: Awaited<ReturnType<typeof loadCompatibilityAssetsV1>> | undefined;
  let execution: Awaited<ReturnType<typeof executeReceiptBoundCompatibilityV1>>;
  try {
    execution = await executeReceiptBoundCompatibilityV1({
      async verifyBeforeProvider() {
        let health: Awaited<ReturnType<typeof runner.run>>;
        try {
          health = await runner.run(createBridgeRequest(
            "health",
        ...
        if (health.status !== "ok" || health.result.workspace_verified !== true ||
          ...
          throw createOperatorFailureV1(
            `Finance bridge pre-provider health refused: ${category}`,
            {
              category: "BRIDGE_REFUSED",
              phase: "health",
              timer_layer: "none",
        ...
        const preProviderArtifacts = await runOperatorPhase(
          "artifact",
          "ARTIFACT_REFUSED",
          async () => await verifyArtifacts(evidenceParams),
        );
        const preProviderHostRuntime = await runOperatorPhase(
          "config",
          "CONFIG_REFUSED",
          async () => resolveReviewedFinanceHostRuntimeV2(params.api.runtime),
        );
        await runOperatorPhase("config", "CONFIG_REFUSED", () => {
          ...
          validateReviewedPluginSourcesV2(
            preProviderHostRuntime.hostConfig,
            params.api.rootDir!,
            openclawArtifactRoot,
            preProviderHostRuntime.codexPluginSource,
            preProviderHostRuntime.openaiPluginSource,
          );
        });
        const preProviderProjection = await runOperatorPhase("config", "CONFIG_REFUSED", () => (
          deriveFinanceAgentProjectionV2(
            preProviderHostRuntime.hostConfig,
            config.agentProfileV2,
            params.api.runtime.version,
        ...
        assets = await runOperatorPhase("artifact", "ARTIFACT_REFUSED", async () => (
          await loadCompatibilityAssetsV1(config.coreDistributionRoot)
        ));
      },
      ...
      async verifyBeforeReceipt() {
        const finalArtifacts = await runOperatorPhase(
          "artifact",
          "ARTIFACT_REFUSED",
          async () => await verifyArtifacts(evidenceParams),
        );
        ...
        const finalAssets = await runOperatorPhase("artifact", "ARTIFACT_REFUSED", async () => (
          await loadCompatibilityAssetsV1(config.coreDistributionRoot)
        ));
        const finalHostRuntime = await runOperatorPhase(
          "config",
          "CONFIG_REFUSED",
          async () => resolveReviewedFinanceHostRuntimeV2(params.api.runtime),
        );
        ...
        await runOperatorPhase("config", "CONFIG_REFUSED", () => {
          ...
          validateReviewedPluginSourcesV2(
            ...
            params.api.rootDir!,
            openclawArtifactRoot,
            finalCodexPluginSource,
            finalOpenAiPluginSource,
          );
        });
        const finalProjection = await runOperatorPhase("config", "CONFIG_REFUSED", () => (
          deriveFinanceAgentProjectionV2(
            finalHostConfig,
            config.agentProfileV2,
            params.api.runtime.version,
        ...
        if (assets === undefined || finalAssets.identity_sha256 !== assets.identity_sha256) {
          throw createOperatorFailureV1(
            "Compatibility assets or runtime evidence changed during evaluation.",
            {
              category: "ARTIFACT_REFUSED",
              phase: "artifact",
              timer_layer: "none",
...
export function registerOperatorCliV1(
  api: OpenClawPluginApi,
  dependencies: OperatorCliDependenciesV1 = defaultDependencies,
): void {
  api.registerCli(({ program }) => {
    ...
    root
      ...
      .requiredOption(
        "--kind <kind>",
        "openclaw_package, finance_plugin_build, or gate5_rehearsal_observer",
      )
      .requiredOption("--root <absolute-path>", "absolute artifact root")
      .action(async (options: { kind: string; root: string }) => {
        ...
        const result = await computeArtifactHashV1(options.kind as ArtifactKind, options.root);
