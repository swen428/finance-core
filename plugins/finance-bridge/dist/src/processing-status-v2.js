...
export function parseProcessingStatusV2(value) {
    const expected = [
    ...
    const intakePublicId = nullableString(value.intake_public_id, /^(?:raw_intake_bridge_[0-9a-f]{32}|raw_intake_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/u);
    if (intakePublicId === null)
        throw new Error("AI processing intake identity is missing.");
    ...
    const admissionDecisionPublicId = nullableString(value.admission_decision_public_id, /^aimd_[0-9a-f]{64}$/u);
    const processingPath = safeText(value.processing_path);
    if (processingPath === null)
        throw new Error("AI processing path is missing.");
    const safeReasonCode = safeText(value.safe_reason_code);
    const displayAlias = safeText(value.display_alias);
    let canonicalAttribution = null;
    if (value.canonical_attribution !== null) {
        ...
        const attribution = value.canonical_attribution;
...
export function renderProcessingFooterV2(status) {
    if (!PROCESSING_PATHS.has(status.processingPath))
        return `🛑 ${SAFE_REASONS.status_unavailable}`;
    if (status.processingPath === "no_model")
        return "🧮 未使用模型";
...
export function renderProcessingStatusDetailV2(status) {
    const lines = [
    ...
    if (status.canonicalAttribution !== null) {
        lines.push(`Canonical attribution: ${status.canonicalAttribution.provider}/${status.canonicalAttribution.model}`, `Agent: ${status.canonicalAttribution.agentId}`);
    }
    if (status.receiptPublicId !== null)
        lines.push(`Compatibility receipt: ${status.receiptPublicId}`);
    if (status.attemptPublicId !== null)
        lines.push(`Attempt: ${status.attemptPublicId}`);
