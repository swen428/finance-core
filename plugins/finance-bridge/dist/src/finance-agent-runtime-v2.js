...
import { projectFinanceAgentConfigV2, } from "./agent-profile-projection-v2.js";
...
function getLoadedPluginSourceV2(runtimeValue, pluginId) {
...
export function getLoadedCodexPluginSourceV2(runtimeValue) {
    return getLoadedPluginSourceV2(runtimeValue, "codex");
}
export function getLoadedOpenAIPluginSourceV2(runtimeValue) {
    return getLoadedPluginSourceV2(runtimeValue, "openai");
}
export function resolveReviewedFinanceHostRuntimeV2(runtimeValue) {
    const runtime = record(runtimeValue, "OpenClaw runtime");
    const configRuntime = record(runtime.config, "OpenClaw runtime config API");
    ...
    const codexSource = record(getLoadedCodexPluginSourceV2(runtime), "loaded Codex plugin source");
    const openaiSource = record(getLoadedOpenAIPluginSourceV2(runtime), "loaded OpenAI plugin source");
    ...
    if (currentCodexEntry.enabled !== true)
        throw new Error("Runtime Codex plugin must be enabled.");
    const currentCodexConfig = record(currentCodexEntry.config, "runtime Codex plugin config");
    exact(currentCodexConfig, ["codexDynamicToolsLoading", "codexDynamicToolsExclude"], "runtime Codex plugin config");
    ...
    const currentOpenAiConfig = record(currentOpenAiEntry.config, "runtime OpenAI plugin config");
...
function pluginSourceEntrySha256(value) {
    return createHash("sha256")
        .update("openclaw-plugin-source-entry-v1\0", "utf8")
        .update(JSON.stringify(canonical(value)), "utf8")
        .digest("hex");
}
function requireFinanceToolsDenied(value, label) {
...
function reviewedPluginLoadPaths(plugins) {
...
function reviewedCodexPluginSource(value, expectedRoot) {
...
function reviewedOpenAiPluginSource(value, openclawArtifactRoot) {
...
function rejectChannelOverrides(value) {
    if (value === undefined)
        return;
    const pending = [record(value, "channels")];
...
function normalizeOpenClawAgentId(value, label) {
    if (typeof value !== "string" || value.trim().length === 0) {
        throw new Error(`${label} must be a non-empty string.`);
    }
    const trimmed = value.trim();
    const normalized = trimmed.toLowerCase();
    if (VALID_OPENCLAW_AGENT_ID.test(trimmed))
        return normalized;
    return normalized
...
function rejectFinanceSubagentTargets(value, label) {
    if (value === undefined)
        return;
    ...
    if (allowed.some((agentId) => (agentId.trim() === "*" ||
...
function rejectFinanceBindings(value) {
...
function rejectFinanceWebhookRoutes(value) {
    ...
    if (hooks.allowedAgentIds !== undefined) {
        ...
        if (allowed.includes("*") || allowed.some((agentId) => normalizeOpenClawAgentId(agentId, "hooks.allowedAgentIds entry") === "finance")) {
            throw new Error("OpenClaw webhook Agent routes must not admit finance.");
        }
    }
    if (hooks.mappings === undefined)
        return;
    if (!Array.isArray(hooks.mappings))
        throw new Error("hooks.mappings must be an array.");
...
function extractFinanceAgent(hostConfigValue) {
    ...
    if (allowed.length !== 4 || allowed[0] !== "telegram" ||
        ...
        allowed[3] !== "openai") {
        throw new Error("plugins.allow must be the reviewed Telegram, Finance, Codex, and OpenAI list.");
    }
    ...
    if (codexEntry.enabled !== true)
        throw new Error("Codex plugin must be enabled.");
    const openaiEntry = record(entries.openai, "OpenAI plugin entry");
    exact(openaiEntry, ["enabled"], "OpenAI plugin entry");
    if (openaiEntry.enabled !== true)
        throw new Error("OpenAI plugin must be enabled.");
    const financeEntry = record(entries["finance-bridge"], "finance plugin entry");
    if (financeEntry.enabled !== true)
        throw new Error("Finance plugin must be enabled.");
    ...
    const financeAgents = configuredAgents.filter((candidate) => normalizeOpenClawAgentId(candidate.id, "configured Agent id") === "finance");
    if (financeAgents.length !== 1)
        throw new Error("Exactly one finance Agent is required.");
    ...
    for (const configured of configuredAgents) {
        ...
        rejectFinanceSubagentTargets(configured.subagents, `Agent ${String(configured.id)} subagents policy`);
    }
...
export function deriveFinanceAgentProjectionV2(hostConfig, evidence, openclawVersion, loadedCodexPluginSource, loadedOpenAiPluginSource) {
    const extracted = extractFinanceAgent(hostConfig);
    const codexPluginSource = reviewedCodexPluginSource(loadedCodexPluginSource, extracted.pluginLoadPaths[1]);
    const openaiPluginSource = reviewedOpenAiPluginSource(loadedOpenAiPluginSource, resolve(extracted.pluginLoadPaths[1], "..", "..", ".."));
...
export function financeProjectionOrRefusalV2(hostConfig, evidence, openclawVersion, loadedCodexPluginSource, loadedOpenAiPluginSource) {
    try {
        return deriveFinanceAgentProjectionV2(hostConfig, evidence, openclawVersion, loadedCodexPluginSource, loadedOpenAiPluginSource);
    }
    catch {
        return {
...
export function validateFinanceHostPolicyV2(hostConfig) {
    extractFinanceAgent(hostConfig);
}
export function validateLoadedFinancePluginRootV2(hostConfig, loadedPluginRoot) {
    const extracted = extractFinanceAgent(hostConfig);
...
export function validateReviewedPluginSourcesV2(hostConfig, loadedFinancePluginRoot, openclawArtifactRoot, loadedCodexPluginSource, loadedOpenAiPluginSource) {
    ...
    const extracted = extractFinanceAgent(hostConfig);
    const reviewedCodexPluginRoot = join(openclawArtifactRoot, "node_modules", "@openclaw", "codex");
