...
const OBSERVER_SOURCE_PATH_SET = new Set(OBSERVER_SOURCE_PATHS);
...
async function collectPath(root, relativePath, files) {
    ...
    if (info.isSymbolicLink())
        throw new Error(`Artifact path is a symlink: ${relativePath}`);
    if (info.isFile()) {
    ...
    while (pending.length > 0) {
        ...
        for await (const child of handle) {
            ...
            if (isExecutableBinDirectory(childRelative))
                continue;
    ...
    if (files.length === initialCount)
        throw new Error(`Artifact directory is empty: ${relativePath}`);
}
function sameIdentity(left, right) {
    return left.dev === right.dev && left.ino === right.ino && left.size === right.size &&
        left.mode === right.mode && left.mtimeNs === right.mtimeNs && left.ctimeNs === right.ctimeNs;
}
async function hashStableFile(root, path, remainingBytes, requireNonempty, captureIdentityBytes) {
    ...
    try {
        ...
        const identityChunks = captureIdentityBytes ? [] : undefined;
...
function requireIdentityBytes(identityBytes, path) {
    const bytes = identityBytes.get(path);
    if (bytes === undefined)
        throw new Error(`Artifact identity bytes are missing: ${path}`);
    return bytes;
}
function requirePackageIdentity(identityBytes, kind) {
    const parsed = JSON.parse(requireIdentityBytes(identityBytes, "package.json").toString("utf8"));
...
export async function computeBuildSourceIdentityV1(rootValue) {
    const root = await requireArtifactRoot(rootValue);
    const collected = [];
    for (const path of BUILD_SOURCE_PATHS)
        await collectPath(root, path, collected);
    const paths = [...new Set(collected)].filter((path) => !OBSERVER_SOURCE_PATH_SET.has(slashPath(path))).sort((left, right) => (compareUtf8(slashPath(left), slashPath(right))));
    const entries = [];
    let byteCount = 0;
    for (const path of paths) {
        const { entry } = await hashStableFile(root, path, MAX_BYTES - byteCount, true, false);
    ...
    return {
        ...material,
        source_identity_sha256: sha256(`${BUILD_SOURCE_POLICY_VERSION}\0${canonicalManifest(material)}`),
        file_count: entries.length,
        byte_count: byteCount,
    };
}
export async function computeObserverHashV1(rootValue) {
    const root = await requireArtifactRoot(rootValue);
    const entries = [];
    let byteCount = 0;
    ...
    return {
        ...material,
        observer_sha256: sha256(`${OBSERVER_SOURCE_VERSION}\0${canonicalManifest(material)}`),
        file_count: entries.length,
        byte_count: byteCount,
    };
}
function requireBuildProvenance(identityBytes, sourceIdentity) {
    const parsed = JSON.parse(requireIdentityBytes(identityBytes, BUILD_PROVENANCE_PATH).toString("utf8"));
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
...
function requireOpenClawAiIdentity(identityBytes) {
    const rootManifest = JSON.parse(requireIdentityBytes(identityBytes, "package.json").toString("utf8"));
    const rootShrinkwrap = JSON.parse(requireIdentityBytes(identityBytes, "npm-shrinkwrap.json").toString("utf8"));
    const aiManifest = JSON.parse(requireIdentityBytes(identityBytes, "node_modules/@openclaw/ai/package.json").toString("utf8"));
    const aiShrinkwrap = JSON.parse(requireIdentityBytes(identityBytes, "node_modules/@openclaw/ai/npm-shrinkwrap.json").toString("utf8"));
    ...
    const rootRecord = rootManifest;
    const shrinkwrapRecord = rootShrinkwrap;
    const aiRecord = aiManifest;
    const aiShrinkwrapRecord = aiShrinkwrap;
    ...
    const shrinkwrapAi = typeof packages === "object" && packages !== null && !Array.isArray(packages)
        ? packages["node_modules/@openclaw/ai"]
        : undefined;
    ...
    const aiShrinkwrapRootRecord = typeof aiShrinkwrapRoot === "object" &&
        ...
        ? aiShrinkwrapRoot
        : undefined;
    const normalizedDependencyMap = (value) => {
        if (typeof value !== "object" || value === null || Array.isArray(value))
            return undefined;
        const entries = Object.entries(value);
        if (entries.some(([, spec]) => typeof spec !== "string"))
            return undefined;
        return entries.toSorted(([left], [right]) => left.localeCompare(right));
    };
    if (shrinkwrapRecord.lockfileVersion !== 3 ||
...
async function requireCodexPluginIdentity(root, identityBytes) {
    ...
    const packageJson = JSON.parse(requireIdentityBytes(identityBytes, "node_modules/@openclaw/codex/package.json").toString("utf8"));
    const shrinkwrap = JSON.parse(requireIdentityBytes(identityBytes, "node_modules/@openclaw/codex/npm-shrinkwrap.json").toString("utf8"));
    const manifest = JSON.parse(requireIdentityBytes(identityBytes, "node_modules/@openclaw/codex/openclaw.plugin.json").toString("utf8"));
    ...
    const shrinkwrapRecord = shrinkwrap;
    ...
    const shrinkwrapRoot = typeof shrinkwrapPackages === "object" &&
        ...
        typeof shrinkwrapPackages[""] === "object" &&
        shrinkwrapPackages[""] !== null &&
        !Array.isArray(shrinkwrapPackages[""])
        ? shrinkwrapPackages[""]
        : undefined;
    const normalizedDependencies = (value, label) => {
        if (value === undefined)
            return {};
        if (typeof value !== "object" || value === null || Array.isArray(value)) {
...
export async function resolveOpenClawArtifactRootV1(argv1Value) {
    if (!isAbsolute(argv1Value))
        throw new Error("OpenClaw argv[1] must be an absolute path.");
    ...
    for (let depth = 0; depth < 12; depth += 1) {
        ...
        if (parent === current || current.endsWith(`${sep}node_modules`))
            break;
        current = parent;
    }
    throw new Error("The running CLI entrypoint is not inside an OpenClaw package root.");
}
export async function computeArtifactHashV1(kind, rootValue) {
    if (!(kind in REQUIRED_PATHS))
        throw new Error("Artifact kind is invalid.");
    const root = await requireArtifactRoot(rootValue);
    const collected = [];
    for (const path of REQUIRED_PATHS[kind])
        await collectPath(root, path, collected);
    const paths = [...new Set(collected)].sort((left, right) => (compareUtf8(slashPath(left), slashPath(right))));
    ...
    for (const path of paths) {
        ...
        const { entry, identityBytes: capturedBytes } = await hashStableFile(root, path, MAX_BYTES - byteCount, NONEMPTY_FILES[kind].has(normalizedPath), IDENTITY_FILES[kind].has(normalizedPath));
        if (capturedBytes !== undefined)
            identityBytes.set(normalizedPath, capturedBytes);
        byteCount += entry.byte_count;
        entries.push(entry);
    }
    const packageVersion = requirePackageIdentity(identityBytes, kind);
    let sourceIdentitySha256 = null;
    if (kind === "openclaw_package") {
        requireOpenClawAiIdentity(identityBytes);
        await requireCodexPluginIdentity(root, identityBytes);
    }
    else {
        sourceIdentitySha256 = requireBuildProvenance(identityBytes, await computeBuildSourceIdentityV1(root));
    }
    const material = {
    ...
    return {
        ...
        artifact_sha256: sha256(`finance-runtime-artifact-tree-v1\0${canonicalManifest(material)}`),
        file_count: entries.length,
        byte_count: byteCount,
