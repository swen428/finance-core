...
export class ReceiptMediaUnavailableError extends Error {
    originalFilename;
    declaredMimeType;
    constructor(message, originalFilename, declaredMimeType, options) {
...
function boundedOriginalFilename(value) {
    if (value === undefined)
        return undefined;
    if (value.trim().length === 0)
        throw new Error("Receipt original filename is empty.");
    let characters = 0;
    ...
    if (characters > 255)
        throw new Error("Receipt original filename exceeds its bounded limit.");
    return value;
}
function singleMedia(metadata) {
    ...
    if (metadata.mediaStagingPending === true)
        throw new Error("Receipt media is still pending.");
    ...
    if (urls !== undefined && urls.length !== 1)
        throw new Error("Exactly one receipt attachment is required.");
    ...
    if (typeof url !== "string")
        throw new Error("Exactly one receipt media URL is required.");
...
function opaqueMediaId(url) {
    const prefix = "media://inbound/";
    if (!url.startsWith(prefix))
        throw new Error("Receipt media must use media://inbound/.");
    let decoded;
    try {
...
export class ReceiptMediaAdapter {
    ...
    async acquire(value, timeoutMs = this.readTimeoutMs) {
        ...
        if (!isRecord(value))
            throw new Error("Receipt attachment metadata is invalid.");
        ...
        try {
            ...
            if (error.code === "ENOENT") {
                throw new ReceiptMediaUnavailableError("Receipt media is no longer available.", originalFilename, descriptor.mime, { cause: error });
            }
            throw error;
        }
        let mediaFd;
