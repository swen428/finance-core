...
function ingressFromEvent(event) {
    return event.financeIngress;
}
...
function validateTurn(event, context) {
    ...
    if (required.some((key) => candidate[key] === undefined) ||
        ...
        !Number.isSafeInteger(candidate.updateId) || candidate.updateId < 0 ||
        ...
        !Number.isSafeInteger(event.timestamp) || event.timestamp % 1_000 !== 0 ||
        event.timestamp < 1_262_304_000_000 ||
        event.timestamp > 253_402_300_799_000)
        return undefined;
    ...
    if (chatId === undefined || messageId === undefined || senderId !== chatId)
        return undefined;
    const photo = candidate.attachmentSha256 !== undefined;
    if (photo && (typeof candidate.attachmentSha256 !== "string" || !SHA256.test(candidate.attachmentSha256)))
        return undefined;
    if (!photo && (candidate.attachmentUnavailable !== undefined || event.content.trim().length === 0))
        return undefined;
    ...
    return {
        ...
        messageId,
        senderId,
        date: event.timestamp / 1_000,
        text: event.content,
        photo,
    };
}
function coreIdentity(ingress) {
    return {
...
function ingressDigest(identity) {
...
function caption(text) {
    ...
    if (text.trim().length === 0)
        throw new Error("Receipt caption is invalid.");
    let count = 0;
...
export class TrustedIngressCapture {
    ...
    constructor(workspaceRoot, runner, media, handoff) {
        this.workspaceRoot = workspaceRoot;
        this.runner = runner;
        this.media = media;
        this.handoff = handoff;
    }
    async verifyCoreCustody(claim, deadlineAt) {
    ...
    async resumePendingReclaims(maxDurationMs = RECLAIM_PASS_DEADLINE_MS) {
        ...
        for (const claim of await this.handoff.pendingReclaims()) {
            ...
            await this.handoff.reclaimVerified(claim, async (candidate) => await this.verifyCoreCustody(candidate, deadline), deadline);
            if (performance.now() >= deadline) {
                throw new Error("Trusted handoff recovery deadline exceeded.");
            }
        }
    ...
    async handle(event, context) {
        ...
        try {
            if (turn.photo)
                receiptCaption = caption(turn.text);
        }
        catch {
            return { handled: false };
        }
        ...
        if (this.inFlight >= MAX_IN_FLIGHT_CAPTURES)
            return { handled: false };
        let release;
        this.activeByMessage.set(messageKey, new Promise((resolve) => { release = resolve; }));
        this.inFlight += 1;
        try {
                ...
                if (adoption !== undefined) {
                    if (!turn.photo) {
                        ...
                        if (routeResponse.status !== "ok" || routeResponse.result.found !== true ||
                            routeResponse.result.final_transaction_created !== false)
                            return {};
                        ...
                        if (!isRecord(route) || !isRecord(routeJob) || !isRecord(statusJob) ||
                            ...
                            routeJob.attachment_content_hash !== null)
                            return {};
                    }
                    return { adoption };
                }
            ...
            const discover = async () => {
                ...
                if (response.status !== "ok")
                    throw new Error("Capture discovery failed.");
            ...
            if (found.found && found.jobId !== undefined) {
                ...
                if (existing.adoption !== undefined)
                    return turn.photo
            ...
            if (turn.photo) {
                ...
                try {
                    capturedJobId = await this.handoff.withPublished(key, intakeId, media, async (published, payloadFd) => {
                        ...
                        let response;
                        try {
                            response = await this.runner.run(request, COMMAND_DEADLINE_MS, payloadFd);
                        }
                        catch {
                            return undefined;
                        }
                ...
                try {
                    ...
                    if (response.status !== "ok")
                        throw new Error("Core text capture refused.");
                    const job = response.result.capture_job;
                    if (!isRecord(job) || typeof job.public_id !== "string" || !JOB_ID.test(job.public_id)) {
