...
def test_authoritative_manifest_contains_migration_001_through_048_in_order() -> None:
    """The current authoritative manifest has ordered migration IDs 001–048
    with no gaps, duplicates or reordered entries."""
    assert len(TEMP_DB_MIGRATION_PATHS) == 48
...
def test_fresh_migration_001_through_048_replay_is_deterministic() -> None:
    """A fresh temporary database must apply 001–048, record all ledger rows
    ...
    try:
        ...
        rows = migration_ledger_rows(conn)
        assert len(rows) == 48
        assert rows[-1]["migration_id"] == "048"
        assert all(row["adoption_mode"] == "applied" for row in rows)
        verify_migration_history(conn, TEMP_DB_MIGRATION_PATHS)
        assert conn.execute("PRAGMA foreign_key_check").fetchall() == []
        assert conn.execute("PRAGMA integrity_check").fetchone()[0] == "ok"
