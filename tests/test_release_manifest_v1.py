...
def _write_fake_packages(artifacts: Path, *, bridge_version: str = "0.1.5") -> None:
    payloads = {
        "finance_core-0.1.5-py3-none-any.whl": b"not-a-wheel",
        "finance_core-0.1.5.tar.gz": b"not-an-sdist",
...
def _rewrite_wheel(wheel: Path, updates: dict[str, bytes]) -> None:
    ...
    record_name = "finance_core-0.1.5.dist-info/RECORD"
...
def _write_bridge_source(source_root: Path) -> dict[str, bytes]:
    ...
    package_manifest = json.dumps(
        {"name": "@finance-codex/finance-bridge", "version": "0.1.5"},
        separators=(",", ":"),
    ).encode()
    source_files = {
        "binding.gyp": b"{}\n",
...
def _write_valid_release_fixture(tmp_path: Path) -> tuple[Path, Path, str]:
    ...
    artifacts.mkdir()
    wheel = artifacts / "finance_core-0.1.5-py3-none-any.whl"
    metadata = (
    ...
    dist_root = "finance_core-0.1.5.dist-info"
    wheel_payloads = {
        **package_payloads,
    ...
    with tarfile.open(artifacts / "finance_core-0.1.5.tar.gz", "w:gz") as archive:
        _add_tar_directory(archive, "finance_core-0.1.5/finance_core/")
        for name, payload in sorted(sdist_payloads.items()):
            _add_tar_file(archive, f"finance_core-0.1.5/{name}", payload)

    with tarfile.open(artifacts / "finance-codex-finance-bridge-0.1.5.tgz", "w:gz") as archive:
...
def test_release_manifest_rejects_modified_build_metadata(tmp_path: Path) -> None:
    artifacts, source_root, core_commit = _write_valid_release_fixture(tmp_path)
    sdist = artifacts / "finance_core-0.1.5.tar.gz"
    ...
    pyproject_name = "finance_core-0.1.5/pyproject.toml"
...
@pytest.mark.parametrize(
    ...
    ["pypdf==6.14.2", "pypdf>=6.16.1", "pypdf @ https://example.invalid/package.whl"],
)
def test_release_manifest_rejects_unapproved_runtime_dependency(
    tmp_path: Path, unapproved_dependency: str
) -> None:
    artifacts, source_root, core_commit = _write_valid_release_fixture(tmp_path)
    project = source_root / "pyproject.toml"
    project.write_text(
        project.read_text(encoding="utf-8").replace("pypdf==6.16.1", unapproved_dependency),
        encoding="utf-8",
    )
    completed = subprocess.run(
        _manifest_command(artifacts, source_root=source_root, core_commit=core_commit),
        capture_output=True,
        text=True,
    )
    assert completed.returncode != 0
...
def test_release_manifest_rejects_unexpected_wheel_entry_point(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(wheel, "a") as archive:
        archive.writestr(
            "finance_core-0.1.5.dist-info/entry_points.txt",
...
def test_release_manifest_rejects_unapproved_python_metadata_header(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(wheel) as archive:
        payloads = {name: archive.read(name) for name in archive.namelist()}
    metadata_name = "finance_core-0.1.5.dist-info/METADATA"
    record_name = "finance_core-0.1.5.dist-info/RECORD"
...
def test_release_manifest_rejects_sdist_sources_inventory_drift(tmp_path: Path) -> None:
    ...
    sources_name = "finance_core-0.1.5/finance_core.egg-info/SOURCES.txt"
...
def test_release_manifest_rejects_bridge_payload_not_in_exact_source(tmp_path: Path) -> None:
    artifacts, source_root, core_commit = _write_valid_release_fixture(tmp_path)
    bridge_path = artifacts / "finance-codex-finance-bridge-0.1.5.tgz"
...
def test_release_manifest_rejects_version_or_artifact_ambiguity(tmp_path: Path) -> None:
    artifacts = tmp_path / "artifacts"
    artifacts.mkdir()
    _write_fake_packages(artifacts, bridge_version="0.2.0")
    ...
    assert completed.returncode != 0
    assert "expected exactly one Bridge package for version 0.1.5" in completed.stderr
